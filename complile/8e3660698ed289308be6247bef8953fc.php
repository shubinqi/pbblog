...
<div class="text-right position mb-3 d-none d-md-block">当前位置：{pboot:position separator='> '}</div>
    ...
    <!-- 标签云 -->
    <div class="panel panel-default wow fadeInUp" data-wow-delay="100ms" data-wow-duration="1s">
        <div class="panel-heading">
            <h1 class="panel-title fs-18 fw700">标签云-站内</h1>
        </div>
        <div class="panel-body label">
            <!-- 标签-->
            {pboot:list scode=2,5 num=20 order=sorting}
                {pboot:tags id=[list:id] scode={sort:tcode}}
                    <a href="[tags:link]"><span class="btn btn-bbq-info">[tags:text]</span></a>
                {/pboot:tags}
            {/pboot:list}
        </div>
    </div>
    ...
    <div class="panel panel-default wow fadeInUp" data-wow-delay="100ms" data-wow-duration="1s">
        <div class="panel-heading">
            <h1 class="panel-title fs-18 fw700">前端学习-站外链接</h1>
        </div>
        <div class="panel-body label">
            <!-- 标签-->
            {pboot:link gid=1 num=50}
                <a href="[link:link]" title="[link:name]" target="_blank"><span class="btn btn-bbq-info">[link:name]</span></a>
            {/pboot:link}
        </div>
    </div>
    <!-- 友情链接 -->
    <div class="panel panel-default wow fadeInUp" data-wow-delay="100ms" data-wow-duration="1s">
        <div class="panel-heading">
            <h1 class="panel-title fs-18 fw700">好友位-友链</h1>
        </div>
        <div class="panel-body label">
            <!-- 友链-->
            {pboot:link gid=2}
...
<div class="col300 hidden-xs slidebar">
    ...
    <script>
        $(".page-link-1").click(function(){
            $(".page-1").show().siblings('.row').hide();
            window.scrollTo(0,0);
        });
        $(".page-link-2").click(function(){
            $(".page-2").show().siblings('.row').hide();
            window.scrollTo(0,0);
        });
    </script>
</div>
...
<?php return array (
  ...
  2 => 'C:/Shubinqi/wwwroot/blog.syabq.cn_765tm6/web/template/default/comm/slidebar.html',
